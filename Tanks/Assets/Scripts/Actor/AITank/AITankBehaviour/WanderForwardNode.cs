...
{
    public class WanderForwardNode : Node
    {
        private AITankController _tankController;

        public WanderForwardNode(AITankController tankController)
        {
        ...
        {
            if (Physics.Raycast(_tankController.transform.position, _tankController.transform.forward, out var hitInfo, 1f, LayerMask.NameToLayer("Walls")) || _tankController.HasDestination() == false)
            {
                Vector3 pos;
